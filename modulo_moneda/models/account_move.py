    ...
    def _aplicar_conversion_moneda(self, record, tipo_cambio):
        ...
        try:
            ...
            if diferencia > 0.10:  # Tolerancia de 10 centavos
                _logger.warning(
                    "⚠️ Pequeña diferencia en totales: Líneas $%.2f vs Factura $%.2f", 
                    total_lineas, record.amount_untaxed
                )

            _logger.info(
                "✅ Conversión exitosa para factura %s. %d líneas actualizadas. TC: %.4f", 
            ...
            _logger.error(
                "❌ Error en conversión para factura %s: %s", 
                record.name, str(e)
            )
            # Revertir cambios en caso de error
            record.invoice_line_ids._recompute_tax_lines()
